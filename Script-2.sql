...
INSERT INTO funcionarios_projetos (id_funcionario,id_projeto,horas_trabalhadas,valor_hora) VALUES
	...
	create view projetosEmAndamento as
	select nome from projetos  where status = 'Em andamento';
	
	select * from  projetosEmAndamento;
	
	
delimiter // 
	create procedure adicionarFuncionario(x_nome varchar(50),x_email  varchar(250),x_senha  varchar(50), x_id_departamento int , x_valor_hora decimal(8,2))
	begin
...
delimiter ; 
	call adicionarFuncionario('Aprendiz 3', 'aprendiz3@empresa.com', 'senha123', 8, 30.00);
...
/* delimiter //
create procedure calcular_valor_total_projetos()
begin
	declare horas_trabalhadas int;
	declare valor_hora int;
	declare valor_total int;
	select valor_total from funcionarios_projetos fp;
	set horas_trabalhadas = horas_trabalhadas;
	set valor_hora = valor_hora ;
	set valor_total = horas_trabalhadas * valor_hora;
end//
delimiter ;


call calcular_valor_total_projetos();
*/

delimiter //
create function calcular_valor_total_projeto(id_projeto int)
returns int 
begin
	declare valor_hora int;
	declare horas_trabalhadas int;
	declare total int;
	set valor_hora = valor_hora;
	set horas_trabalhadas = sum(horas_trabalhadas);
	set total = valor_hora * horas_trabalhadas;
return total;
end //
delimiter ;

select calcular_valor_total_projeto(1);
